...
module slave_handshake
  ...
  sequence qValid; @ (posedge clk)valid [*4]; endsequence : qValid
   ...
   property p_handshake;
      @ (posedge clk) disable iff (!reset_n) 
      if (state_r==IDLE)
        $past(reset_n) || (qValid.triggered)
      else
        if (state_r==REQUEST)  $past(req) && $past(state_r==IDLE)
        else
          if (state_r==ACQUISITION)
            ($past(state_r==REQUEST) && $past(ack)) || 
              $past(state_r==ACQUISITION) && !(qValid.triggered);
    endproperty : p_handshake
      ap_handshake :  assert property (p_handshake);
default clocking @ (posedge clk); endclocking //CVC change added current line 
