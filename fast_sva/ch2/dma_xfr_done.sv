module dmaxfrdone;
  ...
  sequence s_xfr2done; xfr [* 0: 4] ##1 eom; endsequence

  // AF
  // AF ap_som_xfr_done: assert property(@ (posedge clk)  $rose(som) ##1
  // AF                      ((xfr[*0:$] ##1 eom) intersect 1[*1:SIZE) |-> ##1 done;
  ap_som_xfr_done :
  assert property (@(posedge clk) 
    $rose(som) ##1 ((xfr [* 0: $] ##1 eom) intersect
      1 [* 1: SIZE] |-> ##1 done));
  ...
  ap_som_xfr_done2 :
  assert property (@(posedge clk) 
    $rose( som) ##1  (s_xfr2done intersect
      1 [* 1: SIZE]) |-> ##1 done);  // AF fix missing )
  ...
  initial begin
    ...
    // som, xfr, eom, done
    {
      ...
      4'b0100,
      4'b0010,
      4'b0001  //11 // AF removed trailing ,
    };
    for (int i = 0; i < 21; i++) begin
      // AF @(posedge clk) ( som, xfr, eom, done) <= field[i];
