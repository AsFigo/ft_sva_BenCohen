...
property p_req2rdy2ack;
    @(posedge clk) $rose(req) ##[1:2] rdy |->  ##[1:2] ack;  
endproperty
ap_req2rdy2ack: assert property( p_req2rdy2ack);  

ap_equiv: assert property( // @ (posedge clk)    
   (@(posedge clk) $rose(req) ##1  rdy |-> ##[1:2] ack ) and  
   (@(posedge clk) $rose(req) ##2 rdy  |-> ##[1:2] ack ) );


ap_req_in2_rdy_ack: assert property(  
    @(posedge clk) $rose(req) ##1 
                   (rdy[->1] intersect 1[*1:2]) |->  ##[1:2] ack); 
...
task automatic  t_req_rdy (inout bit match);  // looking for a 1st match in the antecedent 
    // ($rose(req) ##1 rdy) 
    if($rose(req))  @(posedge clk) if(rdy) match=1;  // at next cycle 
                                   else @(posedge clk) if(rdy) match=1; // at next cycle 
                                                       else match=0;
endtask 

task automatic t_ack (inout bit pass); // ack after 1 cycle
   @(posedge clk) if(ack) pass=1; 
   else @(posedge clk) if(ack) pass=1; 
   else pass=0;    
endtask  
