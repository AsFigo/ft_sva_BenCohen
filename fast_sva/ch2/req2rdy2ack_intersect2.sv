module reardyack;
  ...
  property p_req2rdy2ack;
    @(posedge clk) 
    $rose(req) ##[1:2] rdy |-> ##[1:2] ack;
  endproperty
  ap_req2rdy2ack :
  assert property (p_req2rdy2ack);

  ap_equiv :
  assert property (@(posedge clk) 
    ($rose(req) ##1 rdy |-> ##[1:2] ack) and 
  ...
  task automatic t_req_rdy_ack();  // the property 
    ...
    if (match) begin  //  antecedent thread matches
      ...
      join
      am_reqrdyack :
      assert (pass) $display("%t req rdy ack assertion is PASS", $realtime);
      else $display("%t req rdy ack assertion is FAIL", $realtime);
    end else $display("%t req rdy ack assertion is vacuous", $realtime);
  endtask
