module m;
 ...
 // Need to handle border cases on "a" after the fist rise of enb.
  ...
  ap_a9newa: assert property(@ (posedge clk) $stable( enb) && $rose(a) |-> ##1 !a[*9]);
 
 // if stable enb && a then a==0 9 more times. One "a" every 10
  ap_a9: assert property(@ (posedge clk) $stable( enb)  |->  a[=1] intersect 1[*10]);
 // $stable( enb) && a |-> ##1 !a[*9]);
